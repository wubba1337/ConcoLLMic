def execute_program(timeout: int) -> tuple[str, int]:
    ...
    target_binary = os.environ.get("FRIDA_TARGET_BINARY", "../bin/validate_brackets_plain")
        ...
        for _attempt in range(max(1, frida_max_retries)):
            try:
                ...
                if rendered_trace.strip():
                    # The Frida wrapper may return non-zero due to wrapper/runtime semantics
                    # (e.g., kill-on-exit). For ACE Step-2 we treat successful adaptation as
                    # a successful execution signal and use the adapted trace as source of truth.
                ...
                try:
                    ...
                    if rendered_trace.strip():
                        # If we already captured meaningful adapted trace lines, treat this
                        # as a successful execution despite wrapper timeout behavior.
                        return rendered_trace, 0
                except Exception:
                    pass
                continue
